...
bool AccountRepositoryDB::loadByID(const uint32_t &id, std::unique_ptr<AccountInfo> &acc) {
	auto query = fmt::format("SELECT `id`, `type`, `premdays`, `lastday`, `creation`, `premdays_purchased`, 0 AS `expires`, `otp_secret`, `last_valid_otp` FROM `accounts` WHERE `id` = {}", id);
...
bool AccountRepositoryDB::loadByEmailOrName(bool oldProtocol, const std::string &emailOrName, std::unique_ptr<AccountInfo> &acc) {
	...
	auto query = fmt::format("SELECT `id`, `type`, `premdays`, `lastday`, `creation`, `premdays_purchased`, 0 AS `expires`, `otp_secret`, `last_valid_otp` FROM `accounts` WHERE `{}` = {}", identifier, g_database().escapeString(emailOrName));
	return load(query, acc);
};

bool AccountRepositoryDB::loadBySession(const std::string &sessionKey, std::unique_ptr<AccountInfo> &acc) {
	auto query = fmt::format(
		"SELECT `accounts`.`id`, `type`, `premdays`, `lastday`, `creation`, `premdays_purchased`, `account_sessions`.`expires`, `otp_secret`, `last_valid_otp` "
		"FROM `accounts` "
...
bool AccountRepositoryDB::setLastValidOTP(const uint32_t &id, uint32_t otp) {
	const bool successful = g_database().executeQuery(
		fmt::format("UPDATE `accounts` SET `last_valid_otp` = {} WHERE `id` = {}", otp, id)
	);

	if (!successful) {
		g_logger().error("Error setting account[{}] last_valid_otp to [{}]", id, otp);
	}

	return successful;
};

bool AccountRepositoryDB::getLastValidOTP(const uint32_t &id, uint32_t &otp) {
	auto result = g_database().storeQuery(
		fmt::format("SELECT `last_valid_otp` FROM `accounts` WHERE `id` = {}", id)
	);
	...
	otp = result->getNumber<uint32_t>("last_valid_otp");
	return true;
};
...
bool AccountRepositoryDB::load(const std::string &query, std::unique_ptr<AccountInfo> &acc) {
	...
	acc->otpSecret = result->getString("otp_secret");
	acc->lastValidOTP = result->getNumber<uint32_t>("last_valid_otp");
